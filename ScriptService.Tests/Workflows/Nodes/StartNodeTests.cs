...
namespace ScriptService.Tests.Workflows.Nodes {
    ...
    public class StartNodeTests {
        ...
        public async Task NoParameters() {
            ...
            await node.Execute(new WorkflowInstanceState(null, new StateVariableProvider(variables), s => null, null, null), CancellationToken.None);
        }
        ...
        public void ForcedParameterNotProvided() {
            ...
            Dictionary<string, object> variables = new Dictionary<string, object>();
            Assert.ThrowsAsync<WorkflowException>(() => node.Execute(new WorkflowInstanceState(null, new StateVariableProvider(variables), s => null, null, null), CancellationToken.None));
        }
